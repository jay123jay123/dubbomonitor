...
def DaysAvgSql(tablename,date):
    sql = "select serviceInterface  , method  , failureCount from `%s` where timestamp = '%s'  group by serviceInterface  , method" % (tablename, date)
    #print sql
    return  sql

def TodayAvgSql(tablename,date,serviceInterface,method):
    sql = "select failureCount from `%s` where serviceInterface = '%s' and method = '%s' and timestamp = '%s' order by id desc limit 1" % (tablename, serviceInterface, method ,date)
    return  sql
